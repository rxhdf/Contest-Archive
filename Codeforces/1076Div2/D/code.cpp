...
void run_case() {
    ...
    for (int i = 0; i < n; ++i) {
        long long available_swords = i + 1;
        long long current_difficulty = a[i];

        auto it = upper_bound(pref.begin(), pref.end(), available_swords);
        
        long long levels_completed = distance(pref.begin(), it);

        long long current_score = current_difficulty * levels_completed;
